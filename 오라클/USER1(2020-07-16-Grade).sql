...
-- 지금 1개의 Record 만 보일지라도 이 결과는 리스트(다수) 형으로 출력된다.
SELECT * FROM tbl_student
WHERE st_name = '남동예';

-- PK칼럼에 2개이상으 조건을 부여하여 조회하기
...
-- 1개의 칼럼 값을 범위로 제한하는 SECTION을 할때
...
-- 학번을 다음과 같이 구성
-- 20001 ~ 20100
-- 어떤 칼럼에 값을 정할때 규칙을 만들고 해당 칼럼을 PK 로 삼아서
...
GROUP BY SUBSTR(sc_num, 1, 4) 
ORDER BY 반 -- ORDER BY 는 SELECT 명령문의 절들 중에 가장 끝에 나타 나야한다.
